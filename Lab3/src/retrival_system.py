...
page_header = ['页面标题', '附件个数', '权限要求']
file_header = ['文件名', '所属页面', '权限要求']
...
class Retrieval(QWidget):
    ...
    def __search(self):
        ...
        # 查询计时
        start_time = time()
        self.lines = self.search.search(query, self.get_role(), self.mode) # utils 查询返回
        end_time = time()
        ...
        if self.mode == 'page':
            for row, data in enumerate(self.lines):
                ...
                self.table.setItem(row, 2, QTableWidgetItem('权限级别{:d}'.format(data['authority'])))
        else:
            for row, file in enumerate(self.lines):
                ...
                self.table.setItem(row, 2, QTableWidgetItem('权限级别{:d}'.format(file[2])))
    ...
    def __open(self, item):
        if self.mode == 'page':
            ...
            os.system('cd ../data/craw/attachment/' + data[1] + '&& start '+ file_name)
