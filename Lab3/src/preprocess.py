...
import docx
from ltp import LTP
from utils import STOP_WORDS, DATA_DIR, LTP_MODEL_PATH
    ...
    for i,data in enumerate(needsegs):
        ...
        for file_name in all_file_name:
            if '.doc' not in file_name: # 不处理不是doc/docx格式的文件
                continue
            pre_file_name = ltp.seg([file_name.split('.doc')[0]])[0][0]
            seg_file_name.append(remove_stop_words(stopwords, pre_file_name))
            if '.docx'not in file_name:
                file_name = file_name + 'x' # 读取docx文件
            try:
                file = docx.Document('E:/Courses/8_2022_spring/Information Retrieval/Lab/Lab3/data/craw/attachment/' + web["title"] + '/' + file_name)
                contents = ' '.join([p.text.strip().replace(' ','') for p in file.paragraphs])
                seg_contents = ltp.seg([contents])[0][0]
